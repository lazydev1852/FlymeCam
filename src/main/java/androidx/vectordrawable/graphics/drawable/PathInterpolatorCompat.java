...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f33mX = new float[min];
            this.f34mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f33mX[i2] = fArr[0];
                this.f34mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f33mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f34mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f33mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f34mY[i3] - 1.0f)) <= 1.0E-5d) {
                    int i4 = 0;
                    float f = 0.0f;
                    while (i < min) {
                        int i5 = i4 + 1;
                        float f2 = this.f33mX[i4];
                        if (f2 >= f) {
                            this.f33mX[i] = f2;
                            i++;
                            f = f2;
            ...
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f33mX[i6]);
            sb.append(SystemInfoUtil.COMMA);
            sb.append(this.f34mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f33mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f33mX[i2]) {
                length = i2;
        ...
        float f2 = this.f33mX[length] - this.f33mX[i];
        if (f2 == 0.0f) {
            return this.f34mY[i];
        }
        float f3 = this.f34mY[i];
        return f3 + (((f - this.f33mX[i]) / f2) * (this.f34mY[length] - f3));
    }
}
