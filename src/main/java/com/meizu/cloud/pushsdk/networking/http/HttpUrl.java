...
public class HttpUrl {
    ...
    public static final class Builder {
        ...
        private static String canonicalizeHost(String str, int i, int i2) {
            ...
            if (!percentDecode.contains(SystemInfoUtil.COLON)) {
                return Util.domainToAscii(percentDecode);
            }
            if (!percentDecode.startsWith(Constants.ARRAY_TYPE) || !percentDecode.endsWith("]")) {
            ...
            while (true) {
                if (i >= i2) {
                    ...
                    if (i6 > i2 || !str.regionMatches(i, "::", 0, 2)) {
                        if (i3 != 0) {
                            if (str.regionMatches(i, SystemInfoUtil.COLON, 0, 1)) {
                                i++;
