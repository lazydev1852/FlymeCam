...
public class Util {
    ...
    public static String md5Hex(String str) {
        try {
            return ByteString.m4852of(MessageDigest.getInstance(MessageDigestAlgorithms.MD5).digest(str.getBytes("UTF-8"))).hex();
    ...
    public static String shaBase64(String str) {
        try {
            return ByteString.m4852of(MessageDigest.getInstance(MessageDigestAlgorithms.SHA_1).digest(str.getBytes("UTF-8"))).base64();
        } catch (UnsupportedEncodingException | NoSuchAlgorithmException e) {
            throw new AssertionError(e);
        }
    }
    ...
    public static ByteString sha1(ByteString byteString) {
        try {
            return ByteString.m4852of(MessageDigest.getInstance(MessageDigestAlgorithms.SHA_1).digest(byteString.toByteArray()));
    ...
    public static ByteString sha256(ByteString byteString) {
        try {
            return ByteString.m4852of(MessageDigest.getInstance(MessageDigestAlgorithms.SHA_256).digest(byteString.toByteArray()));
        } catch (NoSuchAlgorithmException e) {
            throw new AssertionError(e);
        }
    }
    ...
    public static String hostHeader(HttpUrl httpUrl, boolean z) {
        String str;
        if (httpUrl.host().contains(SystemInfoUtil.COLON)) {
            str = Constants.ARRAY_TYPE + httpUrl.host() + "]";
        } else {
            str = httpUrl.host();
        }
        ...
        return str + SystemInfoUtil.COLON + httpUrl.port();
    }
