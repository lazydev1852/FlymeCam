...
public class Session {
    ...
    public Session(long j, long j2, TimeUnit timeUnit, Context context2) {
        ...
        if (sessionFromFile == null) {
            ...
            try {
                ...
                Logger.m4855e(TAG, "Exception occurred retrieving session info from file: %s", e.getMessage());
                this.userId = Util.getEventId();
            }
        }
        updateSessionInfo();
        updateAccessedTime();
        Logger.m4856i(TAG, "Tracker Session Object created.", new Object[0]);
    }

    public SelfDescribingJson getSessionContext() {
        Logger.m4856i(TAG, "Getting session context...", new Object[0]);
        updateAccessedTime();
        return new SelfDescribingJson(TrackerConstants.SESSION_SCHEMA, (Object) getSessionValues());
    }

    public void checkAndUpdateSession() {
        long j;
        Logger.m4854d(TAG, "Checking and updating session information.", new Object[0]);
    ...
    public void setIsBackground(boolean z) {
        Logger.m4854d(TAG, "Application is in the background: %s", Boolean.valueOf(z));
    ...
    private void updateSessionInfo() {
        ...
        this.sessionIndex++;
        Logger.m4854d(TAG, "Session information is updated:", new Object[0]);
        Logger.m4854d(TAG, " + Session ID: %s", this.currentSessionId);
        Logger.m4854d(TAG, " + Previous Session ID: %s", this.previousSessionId);
        Logger.m4854d(TAG, " + Session Index: %s", Integer.valueOf(this.sessionIndex));
        saveSessionToFile();
    }
